...
llvm-cov report ./c2_tests -instr-profile="${LLVM_PROFDATA_DIR}/code.profdata" -use-color || die "Could not generate coverage report"
...
if 1>/dev/null 2>/dev/null command -v less; then
	llvm-cov show ./c2_tests -instr-profile="${LLVM_PROFDATA_DIR}/code.profdata" src/*.cpp -use-color | less -SR
else
	warn "Can't show report in less pager, dumping to terminal"
	sleep .5
	llvm-cov show ./c2_tests -instr-profile="${LLVM_PROFDATA_DIR}/code.profdata" src/*.cpp -use-color
fi

# use lcov.sh or gcovr.sh for html output, it's just better imho
# llvm-cov show ./c2_tests -instr-profile="${LLVM_PROFDATA_DIR}/code.profdata" src/*.cpp -use-color --format html > "${LLVM_PROFDATA_DIR}/index.html"
