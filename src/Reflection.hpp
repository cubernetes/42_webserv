...
protected:
  ...
  void reflect() {} // empty by default, in case you're inheritin from this class and do the reflection in another
                    // (often post-periori, i.e. for another class that does not have reflection) way
  ...
  // The automatically implemented reflect() function (which is also a closure, in a way) in the derived class will call
  // those closures which call reflectMember(), which in turn adds the repr closures to the internal map of member.
