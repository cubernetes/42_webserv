...
size_t HttpServer::getIndexOfServerByHost(const string &requestedHost,
    ...
    for (size_t i = 0; i < _servers.size(); ++i) {
        ...
        if (server.port == port && (std::memcmp(&server.ip, &addr, sizeof(addr)) == 0 ||
...
size_t HttpServer::getIndexOfDefaultServer(const struct in_addr &addr,
                                           in_port_t port) const {
    log.debug() << "Searching for a default server for addr:port "
                << repr(sockaddr_in_wrapper(addr, port)) << std::endl;
    ...
    if (exactMatch != _defaultServers.end()) {
        log.debug() << "Found default server with id " << repr(exactMatch->second)
                    << std::endl;
    ...
    if (anyMatch != _defaultServers.end()) {
        log.debug() << "Found default server (using INADDR_ANY/0.0.0.0) with id "
                    << repr(exactMatch->second) << std::endl;
        return anyMatch->second + 1;
    }
    log.debug() << "Could not find any default server for addr:port "
                << repr(sockaddr_in_wrapper(addr, port)) << std::endl;
    return 0;
}
...
size_t HttpServer::findMatchingServer(const string &host, const struct in_addr &addr,
                                      in_port_t port) const {
    log.debug() << "Trying to find matching server for host " << repr(host)
                << " and addr:port " << repr(sockaddr_in_wrapper(addr, port))
                << std::endl;
    ...
    if (idx) {
        log.debug() << "Found direct match for server (id) " << repr(idx - 1)
                    << std::endl;
    ...
    log.debug()
        << "Couldn't match host:addr:port directly, trying to find a default server"
        << std::endl;
...
size_t HttpServer::findMatchingLocation(const Server &server, const string &path) const {
    int bestIdx = -1;
    int bestScore = -1;
    log.debug() << "For path " << repr(path)
                << ": Trying to find a location block in this server: " << repr(server)
                << std::endl;
    for (size_t i = 0; i < server.locations.size(); ++i) {
        LocationCtx loc = server.locations[i];
        const string &locPath = loc.first;
        if (Utils::isPrefix(locPath, path)) {
            log.debug() << "Match: Location path " << repr(locPath)
                        << " is a prefix of path " << repr(path) << std::endl;
            ...
            log.debug() << "Prefix length is " << repr(currentScore)
                        << " and highest prefix length is " << repr(bestScore)
                        << std::endl;
            if (currentScore > bestScore) {
                bestScore = currentScore;
                bestIdx = static_cast<int>(i);
                log.debug() << "Updating highest prefix length to " << repr(currentScore)
                            << " and index to " << repr(bestIdx) << std::endl;
            }
        } else {
            log.debug() << "Location path " << repr(locPath)
                        << " was not a prefix of path " << repr(path) << std::endl;
        }
    }
    if (bestIdx == -1)
        log.error() << "Could not found a location block for path " << repr(path)
                    << ", this should never happen" << std::endl;
    else
        log.debug() << "Found a location for path " << repr(path)
                    << ", index of location block is " << repr(bestIdx) << std::endl;
...
string getHost(const HttpServer::HttpRequest &request) {
    Logger::lastInstance().debug()
        << "Getting host value from request " << repr(request) << std::endl;
    string host;
    bool notFound = true;
    if (request.headers.find("host") != request.headers.end()) {
        if (request.headers.find("host") != request.headers.end()) {
            ...
            if (colonPos != string::npos)
                host = host.substr(
                    0, colonPos); // TODO: @all: why was it (void) casted before?
        }
    }
    if (notFound)
        Logger::lastInstance().debug()
            << "Host header not found in request, leaving empty" << std::endl;
...
const LocationCtx &HttpServer::requestToLocation(int clientSocket,
    ...
    log.debug() << "Converting socket " << repr(clientSocket) << " to sockaddr_in"
                << std::endl;
    ...
    if (!(serverIdx = findMatchingServer(host, addr.sin_addr, addr.sin_port))) {
        ...
        throw runtime_error("Couldn't find any server for hostname '" + repr(host) +
                            "' and addr:port being " +
    ...
    if (!(locationIdx = findMatchingLocation(server, request.path))) {
        sendError(clientSocket, 404, NULL); // TODO: @all: is 404 rlly correct?
        throw runtime_error(string("Could not find a location for path ") +
                            repr(request.path));
    }
    log.debug() << "Request " << repr(request)
                << " was successfully translated to location "
                << repr(server.locations[locationIdx - 1]) << std::endl;
