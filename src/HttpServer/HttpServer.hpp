...
public:
  ...
  struct Server { // Basically just a thin wrapper around ServerCtx, but with some fields transformed for ease of use
                  // and efficiency
    struct in_addr ip; // network byte order // Should multiple listen directive be possible in the future, then this,
  ...
  struct MultPlexFds {
    ...
    MultPlexFds()
        : multPlexType(Constants::defaultMultPlexType), selectFdSet(), selectFds(), pollFds(), epollFds(), fdStates() {}
  };
  struct CgiProcess {
    pid_t pid;
    int readFd;
