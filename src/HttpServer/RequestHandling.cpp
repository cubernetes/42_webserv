...
bool HttpServer::requestIsForCgi(const HttpRequest &request,
    ...
    if (!Utils::isPrefix(cgi_dir, path)) {
        log.debug() << "Request is not for CGI, because cgi_dir directive "
                    << repr(cgi_dir)
                    << " is not a prefix of the request path: " << repr(path)
                    << std::endl;
        return false;
    }
    log.debug() << "Request IS for CGI, because cgi_dir directive " << repr(cgi_dir)
                << " is a prefix of the request path: " << repr(path) << std::endl;
    return true;
}
...
void HttpServer::handleCgiRead(int cgiFd) {
    ...
    log.debug() << "Pipe FD for the stdout of the CGI process: " << repr(cgiFd)
                << std::endl;
    log.debug() << "Remote client socket FD for the CGI process: " << repr(clientSocket)
                << std::endl;
    ...
    log.debug() << "Pipe FD for the stdin of the CGI process: " << repr(process.writeFd)
                << std::endl;

    if (_cgiToClient.count(process.writeFd) > 0 &&
        _cgiToClient[process.writeFd] == clientSocket) {
    ...
    log.debug() << "Calling " << func("read") << punct("()") << " on the CGI pipe FD "
                << repr(cgiFd) << " for " << repr(sizeof(buffer)) << " bytes"
                << std::endl;
    ssize_t bytesRead = ::read(cgiFd, buffer, sizeof(buffer) - 1);
    log.debug() << "Actually read " << repr(bytesRead) << " bytes" << std::endl;
    if (bytesRead < 0) {
        log.debug() << "Error while reading from the CGI pipe FD " << repr(cgiFd) << ": "
                    << ::strerror(errno) << std::endl;
        log.debug() << "Sending " << num("SIGKILL") << " using " << func("kill")
                    << punct("()") << " to the CGI process with PID " << repr(process.pid)
                    << std::endl;
        ...
        log.debug() << "Removing remote client " << repr(clientSocket)
                    << " from clientToCgi map" << std::endl;
        _clientToCgi.erase(
            clientSocket); // TODO: @all couple _cgiToClient and _clientToCgi as as to
                           // never forget to remove one of them if the other is removed
        log.debug() << "Removing CGI pipe FD " << repr(cgiFd) << " from cgiToClient map"
                    << std::endl;
    ...
    if (bytesRead == 0) { // EOF - CGI process finished writing
        ...
        log.debug() << "Calling " << func("waitpid") << punct("()") << " on pid "
                    << repr(process.pid) << " with flags " << num("WNOHANG") << std::endl;
    ...
    log.debug() << "Updating lastActive time for CGI process " << repr(process) << " to "
                << Utils::formattedTimestamp(process.lastActive) << std::endl;
    buffer[bytesRead] = '\0';
    process.totalSize += static_cast<unsigned long>(bytesRead);

    log.debug() << "Total Response size from CGI process " << repr(process) << " is "
                << repr(process.totalSize) << std::endl;
    log.trace() << "The raw data read from the CGI pipe FD: "
                << repr(string(buffer, static_cast<size_t>(bytesRead))) << std::endl;
    if (process.totalSize >
        Constants::cgiMaxResponseBodySize) { // 10GiB limit // TODO: why put a limit?
        log.warn() << "CGI Response size exceeded limit of 10GiB, namely "
                   << repr(process.totalSize) << " (roughly "
    ...
    if (!process.headersSent) {
        log.debug() << "Buffering data from CGI stdout (headers have NOT yet been sent): "
                    << repr(
                           (char *)string(buffer, static_cast<size_t>(bytesRead)).c_str())
                    << std::endl;
        process.response.append(buffer, static_cast<size_t>(bytesRead));
        size_t headerEnd = process.response.find("\r\n\r\n");
        if (headerEnd != string::npos) {
            log.debug() << "Found end of header fields marker, preparing response"
                        << std::endl;
            ...
            process.response.clear();
                   ...
                   Constants::cgiMaxResponseSizeWithoutBody) { // Status line + Headers
                                                               // too long
            log.warn() << "CGI Response size exceeded limit of 10GiB, namely "
                       << repr(process.totalSize) << " (roughly "
                       << Utils::formatSI(process.totalSize) << ")" << std::endl;
            log.debug() << "Sending " << num("SIGKILL") << " using " << func("kill")
                        << punct("()") << " to the CGI process with PID "
            ...
            log.debug() << "Removing CGI pipe FD " << repr(cgiFd)
                        << " from cgiToClient map" << std::endl;
            _cgiToClient.erase(cgiFd); // TODO: @timo: rename _cgiToClient to something
                                       // like _cgiFdToClientFd
            return;
        } else {
            log.debug()
                << "End of header fields marker not yet found, buffering data further"
                << std::endl;
        }
    } else {
        log.debug()
            << "Queueing data for sending from CGI stdout (headers are already sent): "
            << repr((char *)string(buffer, static_cast<size_t>(bytesRead)).c_str())
            << std::endl;
        queueWrite(process.clientSocket, string(buffer, static_cast<size_t>(bytesRead)));
    }
}

bool HttpServer::parseRequestLine(int clientSocket, const string &_line,
                                  HttpRequest &request) {
    string line = _line;
    line.erase(0, line.find_first_not_of(" \t\r\n"));
    line.erase(line.find_last_not_of(" \t\r\n") + 1);
    log.trace() << "Trimmed original line " << repr(_line) << " to this: " << repr(line)
                << std::endl;
    ...
    if (!(iss >> request.method >> request.path >>
          request.httpVersion)) { // TODO: @all: technically, only space is allowed as a
    ...
    if (iss.rdbuf()->in_avail()) { // this is to check that there are no additional fields
        log.debug() << "Failed to parse request line (too many fields): " << repr(line)
                    << std::endl;
        sendError(clientSocket, 400, NULL);
        return false;
    }
...
bool HttpServer::parseHeader(const string &line, HttpRequest &request) {
    ...
    if (colonPos == string::npos) {
        log.warn() << "Failed to parse header (missing colon): " << repr(line)
                   << std::endl;
        return false;
    }
...
void HttpServer::handleDelete(int clientSocket, const HttpRequest &request,
                              const LocationCtx &location) {
    ...
    if (!fileExists) {
        log.debug()
            << "File " << repr(diskPath)
            << " does not exist and therefore cannot be deleted, sending 404 Not Found"
            << std::endl;
        sendError(clientSocket, 404, &location);
    } else if (S_ISDIR(fileStat.st_mode)) {
        log.debug() << "File " << repr(diskPath)
                    << " is a directory and directory deletion is not supported"
                    << std::endl;
        sendString(clientSocket, "Directory deletion is turned off", 403);
    } else if (S_ISREG(fileStat.st_mode)) {
        log.debug() << "File " << repr(diskPath) << " is a regular file, deleting it"
                    << std::endl;
        if (::remove(diskPath.c_str()) < 0) {
            log.warn() << "File " << repr(diskPath) << " could not be deleted"
                       << std::endl;
            sendString(clientSocket, "Failed to delete resource " + request.path, 500);
        } else {
            log.debug() << "File " << repr(diskPath) << " was successfully deleted"
                        << std::endl;
            sendString(clientSocket, "Successfully deleted " + request.path + "\n");
        }
    } else {
        log.debug() << "File " << repr(diskPath)
                    << " is neither a directory nor a regular file, sending 403 Forbidden"
                    << std::endl;
...
static string getFileName(const string &path) {
    Logger::lastInstance().debug()
        << "Getting file name from path " << repr(path) << std::endl;
    size_t pos;
...
void HttpServer::handleUpload(int clientSocket, const HttpRequest &request,
                              const LocationCtx &location, bool overwrite) {
    log.debug() << "Trying to handle file upload for location " << repr(location)
                << std::endl;
    if (!directiveExists(location.second, "upload_dir")) {
    ...
    string diskPath = getFirstDirective(location.second, "root")[0] +
                      getFirstDirective(location.second, "upload_dir")[0] + fileName;
    ...
    if (!of) {
        log.error() << "Failed to open file on path " << repr(diskPath) << " for writing"
                    << std::endl;
    ...
    of.flush();
    of.close();
    log.debug() << "Written " << repr(request.body.length()) << " bytes to file "
                << repr(diskPath) << std::endl;
    sendString(clientSocket, "Successfully uploaded " + request.path + "\n", 201);
}

void HttpServer::handleRequestInternally(int clientSocket, const HttpRequest &request,
                                         const LocationCtx &location) {
    ...
    if (request.method == "GET" || request.method == "HEAD")
        ...
        log.fatal() << "Method " << repr(request.method)
                    << " is not implemented, sending 405 Method Not Allowed" << std::endl;
...
bool HttpServer::methodAllowed(int clientSocket, const HttpRequest &request,
                               const LocationCtx &location) {
    if (!Utils::allUppercase(request.method)) {
        ...
        log.debug() << "Method " << repr(request.method)
                    << " is not allowed since it is not implemented" << std::endl;
        return false;
    }
    ...
    for (Arguments::const_iterator method = allowedMethods.begin();
         method != allowedMethods.end(); ++method) {
        if (*method == request.method) {
            log.debug() << "Method allowed, since " << *method << " == " << request.method
                        << std::endl;
            return true;
        }
    }
    log.debug() << "Method not allowed, since it was not one of the allowed directives"
                << std::endl;
    sendError(clientSocket, 405, &location);
    return false;
}
...
void HttpServer::rewriteRequest(int clientSocket, int statusCode, const string &urlOrText,
    ...
    if (DirectiveValidation::isDoubleQuoted(urlOrText))
        sendString(clientSocket, DirectiveValidation::decodeDoubleQuotedString(urlOrText),
                   statusCode);
...
void HttpServer::handleRequest(int clientSocket, const HttpRequest &request,
    ...
    if (!methodAllowed(clientSocket, request, location))
        ...
        try {
            ...
            for (ArgResults::const_iterator ext = cgiExts.begin(); ext != cgiExts.end();
                 ++ext) {
                string extension = (*ext)[0];
                string program = ext->size() > 1 ? (*ext)[1] : "";

                CgiHandler handler(*this, extension, program, log);
                log.debug() << "Checking if CgiHandler " << repr(handler)
                            << " can handle request " << repr(request) << std::endl;
                if (handler.canHandle(request.path)) {
                    log.debug() << "CgiHandler " << repr(handler)
                                << " can handle request with path " << repr(request.path)
                                << std::endl;
                    handler.execute(clientSocket, request, location);
                    return;
                } else {
                    log.debug() << "CgiHandler " << repr(handler)
                                << " cannot handle request with path "
                                << repr(request.path) << std::endl;
                }
            }
...
void HttpServer::processContentLength(HttpRequest &request) {
    if (request.headers.find("content-length") != request.headers.end())
        request.contentLength =
            static_cast<size_t>(std::atoi(request.headers["content-length"].c_str()));
...
bool HttpServer::validateRequest(const HttpRequest &request, int clientSocket) {
    ...
    if (!methodIsImplemented(request.method)) { // TODO: @timo: make more stuff static
        log.debug() << "Invalid request: Unsupported method: " << repr(request.method)
                    << std::endl;
        sendError(clientSocket, 405, NULL);
        return false;
    }
    ...
    if (request.httpVersion != "HTTP/1.1") {
        log.debug() << "Invalid request: Unsupported HTTP version: "
                    << repr(request.httpVersion) << std::endl;
    ...
    if ((request.method == "POST" || request.method == "PUT") &&
        !request.chunkedTransfer && request.contentLength == 0) {
        log.debug() << "Invalid " << repr(request.method)
                    << " request: No content length or chunked transfer" << std::endl;
        sendError(clientSocket, 400, NULL);
        return false;
    }
    log.debug() << "POST/PUT request is sound (content-length not " << repr(0)
                << " if not a chunked transfer)" << std::endl;
...
size_t HttpServer::getRequestSizeLimit(int clientSocket, const HttpRequest &request) {
    try {
        ...
        size_t limit = Utils::convertSizeToBytes(
            getFirstDirective(loc.second, "client_max_body_size")[0]);
        log.trace() << "Successfully got request size limit: " << repr(limit)
                    << std::endl;
        return limit;
    } catch (const runtime_error &error) {
        log.error() << "Failed to determine client_max_body_size for client socket "
                    << repr(clientSocket) << ", returning default of "
...
void HttpServer::removeClientAndRequest(int clientSocket) {
    ...
    log.debug() << "Removing socket " << repr(clientSocket) << " from pendingRequests"
                << std::endl;
    _pendingRequests.erase(clientSocket);
}

bool HttpServer::checkRequestBodySize(int clientSocket, const HttpRequest &request,
                                      size_t currentSize) {
    log.debug() << "Checking request body size limit" << std::endl;
    size_t sizeLimit = getRequestSizeLimit(clientSocket, request);
    if (currentSize > sizeLimit) {
        log.warn() << "Request body (so far) is too big: " << repr(currentSize)
                   << " bytes but max allowed is " << repr(sizeLimit) << std::endl;
    ...
    log.trace() << "Request body size so far is " << repr(currentSize)
                << " which is OK (not bigger than " << repr(sizeLimit) << ")"
                << std::endl;
    return true;
}

bool HttpServer::checkRequestSizeWithoutBody(int clientSocket, size_t currentSize) {
    log.debug() << "Checking request without body (request line + headers) size limit"
                << std::endl;
    size_t sizeLimit = Constants::clientMaxRequestSizeWithoutBody;
    if (currentSize > sizeLimit) {
        log.warn() << "Request size (without body) (so far) is too big: "
                   << repr(currentSize) << " bytes but max allowed is " << repr(sizeLimit)
                   << std::endl;
        sendError(clientSocket, 413, NULL); // Payload Too Large
        // TODO: @discuss: what about removing clientSocket from _pendingRequests
        // removeClientAndRequest(clientSocket);
    ...
    log.trace() << "Request size (without body) so far is " << repr(currentSize)
                << " which is OK (not bigger than " << repr(sizeLimit) << ")"
                << std::endl;
    return true;
}

bool HttpServer::processRequestHeaders(int clientSocket, HttpRequest &request,
                                       const string &rawData) {
    log.debug() << "Processing request headers" << std::endl;
    size_t headerEnd = rawData.find("\r\n\r\n");
    log.debug() << "Looking for \\r\\n\\r\\n. Found?: " << repr(headerEnd != string::npos)
                << std::endl;
    if (headerEnd == string::npos) {
        log.debug() << "Not processing headers, waiting for more client data to arrive"
                    << std::endl;
    ...
    log.debug() << "Found line consisting only of \\r, finishing header parsing"
                << std::endl;
    ...
    if (headerEnd + 4 < rawData.size()) {
        ...
        log.debug() << "Moved " << repr(request.bytesRead) << " bytes to body"
                    << std::endl;
    } else {
        request.body.clear();
        request.bytesRead = 0;
    ...
    log.debug() << "Updating request parsing state to " << repr(request.state)
                << std::endl;
    ...
    if (!request.chunkedTransfer &&
        ...
        log.debug() << "No body expected, updating request parsing state to "
...
bool HttpServer::processRequestBody(int clientSocket, HttpRequest &request,
                                    const char *buffer, size_t bytesRead) {
    log.debug() << "Processing request body" << std::endl;
    // Append new data to body
    log.debug() << "Appending new data (" << repr(bytesRead)
                << " bytes) to body (currently " << repr(request.body.length())
                << " bytes)" << std::endl;
...
void HttpServer::finalizeRequest(int clientSocket, HttpRequest &request) {
    ...
    try {
        ...
        bound = true;
        log.debug() << "Found location " << repr(location) << " for request "
                    << repr(request) << std::endl;
        ...
        log.error() << "Failed to match request to a server/location context: "
                    << ansi::red(err.what()) << std::endl;
    }
    log.debug() << "Removing socket " << repr(clientSocket) << " from pendingRequests"
                << std::endl;
    _pendingRequests.erase(clientSocket);
...
void HttpServer::handleIncomingData(int clientSocket, const char *buffer,
                                    ssize_t bytesRead) {
    ...
    if (request.state == READING_HEADERS) {
        ...
        string rawData =
            request.temporaryBuffer + string(buffer, static_cast<size_t>(bytesRead));
        ...
        if (processRequestHeaders(clientSocket, request, rawData)) {
            log.debug() << "Headers fully processed. Body size so far: "
                        << repr(request.body.size()) << std::endl;
            log.debug() << "Expected content length: " << repr(request.contentLength)
                        << std::endl;
            request.temporaryBuffer.clear();
        }
    } else if (request.state == READING_BODY) {
        log.debug() << "Reading body. Current size: " << repr(request.bytesRead)
                    << " Expected: " << repr(request.contentLength) << std::endl;
        if (!processRequestBody(clientSocket, request, buffer,
...
void HttpServer::readFromClient(int clientSocket) {
    if (_cgiToClient.find(clientSocket) != _cgiToClient.end()) {
        log.debug()
            << "CGI process has written data to stdout, trying to read it from FD "
            << repr(clientSocket) << std::endl;
        handleCgiRead(clientSocket);
        return;
    }
    log.debug() << "FD " << repr(clientSocket)
                << " is a remote client socket, trying to read "
                << repr(CONSTANTS_CHUNK_SIZE) << " bytes from it" << std::endl;

    char buffer[CONSTANTS_CHUNK_SIZE + 1]; // for NUL termination
    log.debug() << "Calling " << func("recv") << punct("()") << " on FD "
                << repr(clientSocket) << " without any flags into a buffer of size "
                << repr(sizeof(buffer) - 1) << std::endl;
    ssize_t bytesRead = recv(clientSocket, buffer, sizeof(buffer) - 1, 0);

    log.debug() << "Actually read " << repr(bytesRead) << " bytes" << std::endl;
    if (bytesRead <= 0) {
        log.debug() << "Removing client since only " << repr(bytesRead)
