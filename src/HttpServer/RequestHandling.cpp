...
void HttpServer::handleCgiRead(int cgiFd) {
  ...
  if (bytesRead < 0) {
    ...
    _clientToCgi.erase(clientSocket); // TODO: @all couple _cgiToClient and _clientToCgi as as to never forget to remove
                                      // one of them if the other is removed
...
void HttpServer::parseHeaders(std::istringstream &requestStream, string &line, HttpRequest &request) {
  while (std::getline(requestStream, line) && line != "\r") { // TODO: @all: headers might be very hard to parse
