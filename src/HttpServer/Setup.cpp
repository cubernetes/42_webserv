...
static int createTcpListenSocket() {
    ...
    Logger::lastInstance().debug() << "Successfully created socket with file descriptor number " << repr(listeningSocket) << std::endl;

    int opt = 1;
    Logger::lastInstance().debug() << "Setting socket " << repr(listeningSocket) << " options SO_REUSEADDR" << std::endl;
...
void HttpServer::setupListeningSocket(const Server &server) {
    ...
    Logger::lastInstance().debug() << "Creating a pollfd with POLLIN events for socket " << repr(listeningSocket) << std::endl;
    struct pollfd pfd;
...
void HttpServer::setupServers(const Config &config) {
    ...
    for (ServerCtxs::const_iterator serverCtx = config.second.begin(); serverCtx != config.second.end(); ++serverCtx) {
        Server server(serverCtx->first, serverCtx->second, getFirstDirective(serverCtx->first, "server_name"), log, serverId);
