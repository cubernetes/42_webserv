...
void HttpServer::redirectClient(int clientSocket, const string &newUri, int statusCode) {
    log.debug() << "Redirecting client " << repr(clientSocket) << " to new URI "
                << repr(newUri) << " with status code " << repr(statusCode) << std::endl;
    ...
    log.debug() << "Sending redirection response immediately using " << func("send")
                << punct("()") << std::endl;
    log.trace() << "Full response (" << repr(responseStr.length())
                << " bytes): " << repr(responseStr) << std::endl;
...
bool HttpServer::handleDirectoryRedirect(int clientSocket, const string &path) {
    if (path[path.length() - 1] != '/') {
        log.debug() << "Appending a slash '/' to the end of path " << repr(path)
                    << " and then doing a redirect" << std::endl;
        redirectClient(clientSocket, path + "/");
        return true;
    }
    log.debug()
        << "Can't do a directory redirect on a path that already ends with a slash: "
        << repr(path) << std::endl;
    return false;
}
...
bool HttpServer::handleIndexes(int clientSocket, const string &diskPath,
    ...
    log.debug() << "Iterating through the following indexes: " << repr(indexes)
                << std::endl;
    for (Arguments::const_iterator index = indexes.begin(); index != indexes.end();
        ...
        if (handlePathWithoutSlash(clientSocket, indexPath, newRequest, location,
                                   false)) {
            log.debug() << "Index " << repr(*index)
                        << " was successfully handled! Not trying the other indexes"
                        << std::endl;
            return true;
        }
        log.debug() << "Index " << repr(*index) << " could not be handled, skipping"
                    << std::endl;
    }
...
void HttpServer::handlePathWithSlash(int clientSocket, const string &diskPath,
                                     const HttpRequest &request,
                                     const LocationCtx &location, bool sendErrorMsg) {
    struct stat fileStat;
    int fileExists = ::stat(diskPath.c_str(), &fileStat) == 0;

    log.debug() << "Handling disk path " << repr(diskPath) << " with trailing slash"
                << std::endl;
    if (!fileExists) {
        ...
        log.debug() << "Directory " << repr(diskPath) << " is not actually a directory"
                    << std::endl;
        if (sendErrorMsg)
            sendError(clientSocket, 404, &location);
        return;
    } else {
        log.debug() << "Trying to handle index file serving first for directory "
                    << repr(diskPath) << std::endl;
        if (handleIndexes(clientSocket, diskPath, request, location)) {
            return;
        }
        log.debug() << "Couldn't find any index files, trying autoindex next if enabled"
                    << std::endl;
        ...
        log.debug() << "Autoindexing is enabled, indexing directory " << repr(diskPath)
                    << std::endl;
...
bool HttpServer::handlePathWithoutSlash(int clientSocket, const string &diskPath,
                                        const HttpRequest &request,
                                        const LocationCtx &location, bool sendErrorMsg) {
    struct stat fileStat;
    int fileExists = ::stat(diskPath.c_str(), &fileStat) == 0;

    log.debug() << "Handling disk path " << repr(diskPath) << " without trailing slash"
                << std::endl;
    if (!fileExists) {
        ...
        if (!handleDirectoryRedirect(clientSocket, request.path)) {
            log.debug() << "Failed to handle directory redirect, sending 404 Not Found"
                        << std::endl;
            if (sendErrorMsg)
                sendError(clientSocket, 404, &location);
            return false;
...
string HttpServer::determineDiskPath(const HttpRequest &request,
    ...
    log.debug() << "Determining file to server for request " << repr(request)
                << " to location " << repr(location) << std::endl;
    ...
    if (directiveExists(location.second, "alias")) { // TODO: @timo: sanitziation?
        ...
        log.debug() << "Alias directive is present and has the value " << repr(alias)
                    << std::endl;
        string prefix = location.first;
        size_t prefixLen = prefix.length();
        string path = request.path;
        pathWithoutRoot = alias + path.substr(prefixLen);
        log.debug() << "Original path " << repr(path)
                    << " after alias substitution is now " << repr(pathWithoutRoot)
                    << std::endl;
    } else {
        log.debug() << "No alias directive present, using original path "
                    << repr(request.path) << " as relative path to root" << std::endl;
...
void HttpServer::serveStaticContent(int clientSocket, const HttpRequest &request,
                                    const LocationCtx &location) {
    log.debug() << "Trying to serve static content for request " << repr(request)
                << std::endl;
