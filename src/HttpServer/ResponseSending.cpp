...
void HttpServer::writeToClient(int clientSocket) {
    ...
    log.debug() << "Sending this data to fd " << clientSocket << ": "
                << Utils::escape(data) << std::endl;
    ...
    if (bytesSent < 0) {
        ...
        if (_cgiToClient.count(clientSocket) > 0) {
            ...
            log.debug() << "Notifying client " << client
                        << " with a 500, since CGI process died/closed stdin"
                        << std::endl;
            sendError(client, 500, NULL);
        }
        return;
    }
    log.debug() << "Successfully sent " << bytesSent << " bytes to client "
...
void HttpServer::sendError(int clientSocket, int statusCode,
    ...
    if (location == NULL || !sendErrorPage(clientSocket, statusCode, *location)) {
        log.debug() << "Sending hardcoded error with status code: " << statusCode
                    << std::endl;
        string errorContent =
            wrapInHtmlBody("<h1>\r\n\t\t\t" + STR(statusCode) + " " +
                           statusTextFromCode(statusCode) + "\r\n\t\t</h1>");
...
void HttpServer::sendString(int clientSocket, const string &payload, int statusCode,
                            const string &contentType, bool onlyHeaders) {
    log.debug() << "Preparing to send string response with status code: " << statusCode
                << std::endl;
