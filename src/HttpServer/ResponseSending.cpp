...
void HttpServer::terminatePendingCloses(int clientSocket) {
    if (_pendingCloses.find(clientSocket) != _pendingCloses.end()) {
        log.debug() << "Removing client " << repr(clientSocket) << " from pendingCloses"
                    << std::endl;
        _pendingCloses.erase(clientSocket);
        removeClient(clientSocket);
    } else {
        log.debug() << "Client " << repr(clientSocket)
                    << " is not in pendingCloses, not doing anything with it"
                    << std::endl;
    }
}

bool HttpServer::maybeTerminateConnection(PendingWriteMap::iterator it,
                                          int clientSocket) {
    if (it == _pendingWrites.end()) { // no pending writes anymore, maybe close
        if (_clientToCgi.find(clientSocket) ==
            _clientToCgi
                .end()) { // STOP, make sure CGI doesn't want to write data to this client
            if (_cgiToClient.count(clientSocket) ==
                0) { // STOP, make sure clientSocket is not a writeFd to the CGI process
                log.debug() << "Client " << repr(clientSocket)
            ...
            log.debug()
                << "There are no pending writes to FD " << repr(clientSocket)
                << " anymore, but don't terminate "
    ...
    log.debug() << "There are still pending writes, not terminating connection to client "
                << repr(clientSocket) << std::endl;
    return false;
}

void HttpServer::terminateIfNoPendingDataAndNoCgi(PendingWriteMap::iterator &it,
                                                  int clientSocket, ssize_t bytesSent) {
    PendingWrite &pw = it->second;

    // Check whether we've sent everything and that there are no CGI processes
    if (_clientToCgi.find(clientSocket) == _clientToCgi.end() &&
        (pw.length() == 0 || bytesSent == 0)) {
        log.debug() << "Removing client " << repr(clientSocket) << " from pendingWrites"
                    << std::endl;
        ...
        if (_cgiToClient.count(clientSocket) != 0) {
            log.debug() << "Removing CGI pipe FD " << repr(clientSocket)
                        << " (write end, stdin of CGI) from cgiToClient map" << std::endl;
...
void HttpServer::writeToClient(int clientSocket) {
    ...
    if (_cgiToClient.count(clientSocket)) { // it's a writeFd for the CGI, can't use send
        log.debug() << "Writing this data of " << repr(dataSize) << " bytes to socket "
                    << repr(clientSocket) << ": " << repr((char *)data) << std::endl;
        bytesSent = ::write(clientSocket, data, dataSize);
        log.debug() << "Written " << repr(bytesSent) << " bytes to client "
                    << repr(clientSocket) << std::endl;
    } else {
        log.debug() << "Sending this data of " << repr(dataSize) << " bytes to socket "
                    << repr(clientSocket) << ": " << repr((char *)data) << std::endl;
        bytesSent = ::send(clientSocket, data, dataSize, 0);
        log.debug() << "Sent " << repr(bytesSent) << " bytes to client "
                    << repr(clientSocket) << std::endl;
    }
    ...
    if (bytesSent < 0) {
        ...
        if (_cgiToClient.count(clientSocket) > 0) {
            ...
            log.debug() << "Client " << repr(client) << " had data to send but pipe broke"
                        << std::endl;
            log.debug() << "Notifying remote client " << repr(client) << " with a "
                        << repr(500) << ", since CGI process died/closed stdin"
                        << std::endl;
    ...
    log.debug() << "Successfully sent data to " << repr(clientSocket)
                << ", now checking if we need to reset the connection" << std::endl;
...
bool HttpServer::sendFileContent(int clientSocket, const string &filePath,
                                 const LocationCtx &location, int statusCode,
    ...
    log.debug() << "Could open file " << repr(filePath)
                << ", determining file size via istream::seekg and istream::tellg"
                << std::endl;
    ...
    headers << _httpVersionString << " " << statusCode << " "
            << statusTextFromCode(statusCode) << "\r\n"
            << "Content-Length: " << fileSize << "\r\n"
            << "Content-Type: "
            << (contentType.empty() ? getMimeType(filePath) : contentType) << "\r\n"
            << "Connection: close\r\n\r\n";

    queueWrite(clientSocket, headers.str());

    if (!onlyHeaders) {
        log.debug() << "Queueing actual file contents, since it's not a HEAD request"
                    << std::endl;
        ...
        log.debug() << "It was a HEAD request, not queuing any file contents"
                    << std::endl;
    }
    log.debug() << "Adding socket " << repr(clientSocket) << " to pendingCloses"
                << std::endl;
...
string HttpServer::wrapInHtmlBody(const string &text) {
    log.trace() << "Wrapping text in some html and body tags: " << repr(text)
                << std::endl;
    return "<html>\r\n\t<body>\r\n\t\t" + text + "\r\n\t</body>\r\n</html>\r\n";
}
...
static string getErrorPagePath(int statusCode, const LocationCtx &location) {
    Logger::lastInstance().debug()
        << "Getting error page path for status code " << repr(statusCode)
        << " and location " << repr(location) << std::endl;
    ArgResults allErrPageDirectives = getAllDirectives(location.second, "error_page");
    for (ArgResults::const_iterator errPages = allErrPageDirectives.begin();
         errPages != allErrPageDirectives.end(); ++errPages) {
        Arguments::const_iterator code = errPages->begin(), before = --errPages->end();
        for (; code != before; ++code) {
            if (std::atoi(code->c_str()) == statusCode) {
                Logger::lastInstance().debug()
                    << "Returning error page path for status code " << repr(statusCode)
    ...
    Logger::lastInstance().debug()
        << "Status code " << repr(statusCode)
        << " has no error page, returning empty string" << std::endl;
    return "";
}

bool HttpServer::sendErrorPage(int clientSocket, int statusCode,
                               const LocationCtx &location) {
    log.debug() << "Trying to send error page to client " << repr(clientSocket)
                << " with status code " << repr(statusCode) << " and location block "
    ...
    string errorPagePath =
        getFirstDirective(location.second, "root")[0] + errorPageLocation;
    log.debug() << "Full file system error page path is " << repr(errorPagePath)
                << ", trying to open that file" << std::endl;
...
void HttpServer::sendError(int clientSocket, int statusCode,
                           const LocationCtx *const location) {
    if (location == NULL || !sendErrorPage(clientSocket, statusCode, *location)) {
        ...
        log.debug() << "Sending hardcoded error with status code: " << repr(statusCode)
                    << std::endl;
        string errorContent =
            wrapInHtmlBody("<h1>\r\n\t\t\t" + STR(statusCode) + " " +
                           statusTextFromCode(statusCode) + "\r\n\t\t</h1>");
...
void HttpServer::sendString(int clientSocket, const string &payload, int statusCode,
                            const string &contentType, bool onlyHeaders) {
    log.debug() << "Preparing to send string response with status code: "
                << repr(statusCode) << std::endl;
    ...
    response << _httpVersionString << " " << statusCode << " "
             ...
             << "\r\n";
    if (!onlyHeaders) {
        log.debug() << "Queueing actual string payload, since it's not a HEAD request"
                    << std::endl;
        response << payload;
    } else {
