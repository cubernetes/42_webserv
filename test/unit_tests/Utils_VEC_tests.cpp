...
// positive tests
TEST_CASE("empty vec", "[utils][VEC]") { CHECK_THAT(VEC(std::string), IsEmpty()); }
...
TEST_CASE("3 strings", "[utils][VEC]") {
    ...
    CHECK_THAT(VEC(std::string, "teststring", "another string", "and another"),
               RangeEquals(testVec));
}
...
TEST_CASE("2 vecs of some strings", "[utils][VEC]") {
    ...
    CHECK_THAT(VEC(std::vector<std::string>, stringVec, stringVec2),
               RangeEquals(testVec));
}
...
TEST_CASE("nested VEC calls with parentheses -> vec<vec<int>>", "[utils][VEC]") {
    ...
    CHECK_THAT(VEC(std::vector<int>, (VEC(int, 42, 42)), (VEC(int, 21, 21, 21))),
               RangeEquals(testVec));
}
