...
def test_location_directives(webserver, base_url_foo):
    """Test location directives and routing"""
    # Test basic location
    assert requests.get(f"{base_url_foo}/foo").status_code == 200
    
    # Test location with trailing slash
    assert requests.get(f"{base_url_foo}/foo/").status_code == 200

def test_redirects(webserver, base_url_foo):
    """Test redirect functionality"""
    response = requests.get(f"{base_url_foo}/bar", allow_redirects=False)
